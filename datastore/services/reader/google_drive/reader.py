...
class DjangoGoogleDriveReader(GoogleDriveReader):
    ...
    def _get_credentials(self) -> Any:
        ...
        Download the credentials.json file with these instructions:
        https://developers.google.com/drive/api/v3/quickstart/python.
            Copy credentials.json file and rename it to client_secrets.json file which will be used by pydrive for
            downloading files.
            So, we need two files:
                1. credentials.json
                2. client_secrets.json
            Both 1, 2 are esentially same but needed with two different names according to google-api-python-client,
            google-auth-httplib2, google-auth-oauthlib and pydrive libraries.
            ...
            if folder_id:
                ...
                for item in items:
                    if item["mimeType"] == folder_mime_type:
                        fileids_meta.extend(self._get_fileids_meta(folder_id=item["id"], mime_types=mime_types))
                    else:
    ...
    def load_fileids_meta(
        ...
        try:
            ...
            return fieldids_meta_list
        except Exception as e:
            logger.error(f"An error occurred while loading from folder: {e}")

    def load_document_from_fieldids_meta(self, fieldids: dict) -> List[Document]:
        # it is getting this data from the load_fileids_meta method
        # {
        #     "file id": fileid_meta[0],
        #     "author": fileid_meta[1],
        #     "file name": fileid_meta[2],
        ...
        #     "modified at": fileid_meta[5],
        # }
        # it should convert it to the list
